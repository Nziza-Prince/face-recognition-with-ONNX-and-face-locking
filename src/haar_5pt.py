...
try:
    ...
    # Try to use the legacy solutions API first
    try:
        from mediapipe.python.solutions import face_mesh
        USE_LEGACY_API = True
    except ImportError:
        # Fall back to new task-based API
...
class Haar5ptDetector:
    def __init__(
        ...
        # MediaPipe FaceLandmarker - Try legacy API first, then new API
        if mp is None:
            raise RuntimeError(
        ...
        if USE_LEGACY_API:
            try:
                # Use legacy solutions API (works with older MediaPipe versions)
                ...
                if self.debug:
                    print("[haar_5pt] MediaPipe FaceMesh (legacy) initialized successfully")
            except Exception as e:
                if self.debug:
                    print(f"[haar_5pt] Legacy MediaPipe failed: {e}")
                raise RuntimeError(f"Failed to initialize MediaPipe: {e}")
        else:
            # Try new task-based API
            try:
                # Download and use the default face landmarker model
                ...
                if not os.path.exists(model_path):
                    if self.debug:
                        print("[haar_5pt] Downloading face landmarker model...")
                ...
                if self.debug:
                    print("[haar_5pt] MediaPipe FaceLandmarker (new API) initialized successfully")
            except Exception as e:
                if self.debug:
                    print(f"[haar_5pt] New MediaPipe API failed: {e}")
                raise RuntimeError(f"Failed to initialize MediaPipe FaceLandmarker: {e}")
    ...
    def _facemesh_5pt(self, frame_bgr: np.ndarray) -> Optional[np.ndarray]:
        ...
        try:
            ...
            if self.use_legacy:
                ...
                # Extract the 5 key points using the same indices
                idxs = [
                    self.IDX_LEFT_EYE,
                    self.IDX_RIGHT_EYE,
                    self.IDX_NOSE_TIP,
            ...
            if kps[0, 0] > kps[1, 0]:
                kps[[0, 1]] = kps[[1, 0]]  # swap left and right eye
            if kps[3, 0] > kps[4, 0]:
                kps[[3, 4]] = kps[[4, 3]]  # swap left and right mouth

            return kps
            
        except Exception as e:
            if self.debug:
                print(f"[haar_5pt] MediaPipe processing failed: {e}")
            return None
